...
<body class="bg-gray-300 " style="background-color: #d5deef;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.getElementById('add-article-btn').addEventListener('click', function (event) {
                event.preventDefault();
                articleIndex++;
                let container = document.getElementById('articles-container');
                container.appendChild(createArticleFields(articleIndex));
            });

            function createArticleFields(index) {
                let div = document.createElement('div');
                div.classList.add('article-fields', 'border-b', 'border-gray-900/10', 'pb-12');
                div.innerHTML = `
                <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">

<div class="col-span-full">
    <label for="cover-photo"
        class="block text-sm font-medium leading-6 text-gray-900">Article Photo</label>
    <div
        ...
        <div class="text-center">
            ...
            <div class="mt-4 flex text-sm leading-6 text-gray-600">
                <label for="file-upload"
                    ...
                    <input type="file" accept="image/*" name="article_picture[]" id="article_picture-${index}" class="sr-only">
                </label>
            </div>
            <input type="file" accept="image/*" name="article_picture[]" id="article_picture-${index}">
...
<label for="" class="block text-sm font-medium leading-6 text-gray-900">Article
    Category</label>
<div class="mt-2">
<select id="article_category-${index}" name="article_category[]"
...
<label for="article_title" class="block text-sm font-medium leading-6 text-gray-900">Article
    Title</label>
<div class="mt-2">
    <input type="text" name="article_title[]" id="article_title-${index}" autocomplete="street-address"
        class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
</div>
</div>
...
<label for="article_description"
    ...
    Description</label>
<div class="mt-2">
    <input type="text" name="article_description[]" id="article_description-${index}"
